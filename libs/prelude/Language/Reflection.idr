...
||| A quotation of a term with type `a`. Because this datatype is
||| abstract, there's no way to construct it outside of its home
||| module. This scope restriction ensures that only the special
||| references generated by the elaborator work.
abstract data TypedQuote : Type -> Type where
  ||| The hidden constructor for typed quotations.
  MkTypedQuote : {a : Type} -> TT -> TypedQuote a

||| Erase the type from a typed quotation, yielding a `TT`
